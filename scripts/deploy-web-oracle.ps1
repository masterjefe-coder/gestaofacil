...
$repoStateLines = @(& git status --porcelain)
$repoState = ($repoStateLines -join "`n").Trim()
if ($repoState) {
  throw "Há mudanças locais não commitadas. Faça commit antes do deploy."
}
...
& git push --force "ssh://$User@$ServerHost$remoteGitDir" "HEAD:refs/heads/$Branch"
...
if ($pushExitCode -ne 0) {
  throw "Push do código para a VM falhou."
}
